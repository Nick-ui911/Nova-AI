...
export default function ChatWindow({ chatId, setChatId, onNewChat }) {
  ...
  useEffect(() => {
    if (!chatId) {
      setMessages([]);
      isFirstChatRef.current = true; // Reset when no chatId
      return;
    }

    // Skip fetching if this is the first chat being created
    // (messages are already being added via onAppend)
    if (isFirstChatRef.current) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 relative overflow-hidden">
      ...
      <div
        className="absolute bottom-0 right-0 w-96 h-96 bg-amber-600 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-pulse"
              ...
              <div className="relative bg-slate-900/90 backdrop-blur-md rounded-2xl px-8 py-6 shadow-2xl border border-orange-500/20">
                <div className="flex items-center space-x-3">
                  <div className="flex space-x-1.5">
                    <div
                      className="w-3 h-3 bg-gradient-to-r from-orange-500 to-amber-500 rounded-full animate-bounce"
                  ...
                  <div className="px-5 py-3.5 rounded-2xl shadow-lg backdrop-blur-sm bg-slate-800/80 text-slate-100 border border-orange-500/20 shadow-orange-500/10">
                    <div className="flex items-center space-x-2">
                      <div className="flex space-x-1">
                        <div
                          className="w-2 h-2 bg-gradient-to-r from-orange-400 to-amber-400 rounded-full animate-bounce"
                          style={{ animationDelay: "0ms" }}
                        ></div>
                        <div
                          className="w-2 h-2 bg-gradient-to-r from-orange-400 to-amber-400 rounded-full animate-bounce"
